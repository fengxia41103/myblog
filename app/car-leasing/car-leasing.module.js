...
angular
  ...
  .controller("CarLeasingController", [
    ...
    function ($scope) {
      ...
      vm.$watchGroup(["msrp", "invoice", "purchase", "lease"], function () {
        // Discounts from MSRP
        ...
        vm.invoice_purchase_discount =
          ((vm.invoice - vm.purchase) / vm.invoice) * 100;
        vm.invoice_lease_discount =
          ((vm.invoice - vm.lease) / vm.invoice) * 100;
      ...
      // Residue value
      vm.$watchGroup(["msrp", "residue"], function () {
        // Residue value is always computed from MSRP!
        vm.residue_value = (vm.msrp * vm.residue) / 100;
      });

      // MF
      vm.$watchGroup(["apr", "msd_selected", "msd_mf_discount"], function () {
      ...
      // Deal
      vm.$watchGroup(
        ["lease", "sales_tax", "downpayment", "rebates", "credits"],
        function () {
          vm.lease_after_tax = vm.lease * (1 + vm.sales_tax / 100);
          vm.lease_sales_tax = vm.lease_after_tax - vm.lease;
          vm.net_capital_cost =
            vm.lease_after_tax - vm.downpayment - vm.rebates - vm.credits;
        }
      );

      // Depreciation cost
      vm.$watchGroup(
        ["net_capital_cost", "residue_value", "term"],
        function () {
      ...
      // Financing cost
      vm.$watchGroup(
        ["net_capital_cost", "residue_value", "actual_mf"],
        function () {
      ...
      vm.$watchGroup(
        ...
        function () {
          vm.monthly_lease_payment_before_tax =
            vm.monthly_depreciation + vm.monthly_financing;
          vm.monthly_lease_payment_after_tax =
            vm.monthly_lease_payment_before_tax * (1 + vm.monthly_tax / 100);

          // Taxes
          vm.monthly_tax_charge =
            vm.monthly_lease_payment_after_tax -
            vm.monthly_lease_payment_before_tax;
          ...
          vm.total_lease_payment_before_tax =
            vm.term * vm.monthly_lease_payment_before_tax;
          vm.total_lease_payment_after_tax =
            vm.term * vm.monthly_lease_payment_after_tax;
      ...
      vm.$watchGroup(
        [
          ...
          "acquisition_fee",
          "total_msd",
          "monthly_lease_payment_after_tax",
        ],
        function () {
          ...
          vm.upfront_forme =
            vm.downpayment + vm.monthly_lease_payment_after_tax + vm.total_msd;
          vm.upfront_cost =
            vm.upfront_gov +
            vm.upfront_dealer +
            vm.upfront_bank +
      ...
      // Security deposit
      vm.$watchGroup(["security_refund_rate", "security_deposit"], function () {
        vm.security_refund =
          (vm.security_refund_rate / 100) * vm.security_deposit;
      });
      ...
      // Cost of ownership
      vm.$watchGroup(["upfront_cost", "lease_end_cost"], function () {
        vm.cost_of_ownership =
          vm.upfront_cost +
          vm.lease_end_cost +
          (vm.term - 2) * vm.monthly_lease_payment_after_tax;
      ...
      vm.$watchGroup(
        ["residue_value", "dealer_resale_markup", "dealer_ownership_resale"],
        function () {
          vm.resale_price =
            vm.residue_value * (1 + vm.dealer_resale_markup / 100);
          vm.dealer_resale_value =
            (vm.resale_price * vm.dealer_ownership_resale) / 100;
        }
      );

      vm.$watchGroup(
        ["dealer_cogs", "msrp", "invoice", "resale_price"],
        function () {
          vm.dealer_cogs_msrp_discount =
            ((vm.msrp - vm.dealer_cogs) / vm.msrp) * 100;
          vm.dealer_cogs_invoice_discount =
            ((vm.invoice - vm.dealer_cogs) / vm.invoice) * 100;
      ...
      vm.$watchGroup(
        ["upfront_cost", "dealer_cogs", "lease_sales_tax"],
        function () {
          vm.dealer_initial_investment =
            vm.upfront_cost -
            vm.dealer_cogs -
            vm.lease_sales_tax -
            vm.upfront_gov -
      ...
      vm.$watchGroup(["lease_end_cost", "dealer_resale_value"], function () {
        vm.dealer_terminal_value =
          vm.lease_end_cost + vm.dealer_resale_value - vm.monthly_tax_charge;
      });

      vm.$watchGroup(
        [
          "dealer_initial_investment",
          "dealer_terminal_value",
          "dealer_cost_of_capital",
        ],
        function () {
          // Compute IRR
          var a = new Array();
